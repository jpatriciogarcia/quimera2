...
class ReporteActividadEstadoEnEjecucionView {
    ...
    public function showAll($Reporte) {
	    ...
	    <table width="100%">
	      ...
	      <tr>
	        ...
	        <td><td><?php echo FormHelper::selectBox('centro_familiar', '', $CentroFamiliar->Find('1=1 order by nombre'), 'codigo', 'nombre', true, array('class' => 'validate-selection selectArea', 'onChange' => 'updateReport()')); ?></td></td>
        ...
        ?>
        <script>
        parent.Windows.getFocusedWindow().setTitle('Reportes :: Actividades En Ejecución');

        function updateReport() {
            new Ajax.Updater('div-list', '<?php echo WEB_PATH; ?>/ReporteActividadEstadoEnEjecucion/updateList', {
    ...
    public function getList($Reporte) {
        ...
        foreach ($Reporte as $Reporte) {
            $html .= "\n <tr>"
            ."\n  <td><div align='left'>{$Reporte->numeroactividad}</div></td>"
            ."\n  <td><div align='left'>{$Reporte->nombreactividad}</div></td>"
            ."\n  <td><div align='left'>{$Reporte->nombreestado}</div></td>"
            ."\n  <td><div align='left'>{$Reporte->fecha_inicio}</div></td>"
            ."\n  <td><div align='left'>{$Reporte->fecha_termino}</div></td>"
            ."\n  <td><div align='left'>{$Reporte->nombrecentro}</div></td>"
            ."\n </tr>";
        }
        $html .= "\n</table>";
