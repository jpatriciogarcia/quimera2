...
class ReporteActividadExitosaView {
    ...
    public function showAll($Reporte) {
	    ...
	    <table width="100%">
	      ...
	      <tr>
	        ...
	        <td><?php echo FormHelper::selectBox('centro_familiar_nombre', '', $CentroFamiliar->Find('1=1 order by nombre'), 'codigo', 'nombre', true, array('class' => 'validate-selection selectArea', 'onChange' => 'updateReport()')); ?></td>
	        <td><td><?php echo HtmlHelper::imageButton('buscar_ActividadEstadoNueva', "onclick='updateReport()'") ?></td></td>
	        </tr>
        ...
        ?>
        <script>
        parent.Windows.getFocusedWindow().setTitle('Reportes :: Actividades Exitosas');

        function updateReport() {
            new Ajax.Updater('div-list', '<?php echo WEB_PATH; ?>/ReporteActividadExitosa/updateList', {
                parameters: { centro_familiar: $('centro_familiar_nombre').value, fechainicio: $('fecha_inicio').value, fechatermino: $('fecha_termino').value},
    ...
    public function getList($Reporte) {
        ...
        foreach ($Reporte as $Reporte) {
            $html .= "\n <tr>"
            ."\n  <td><div align='left'>{$Reporte->actividad_codigo}</div></td>"
