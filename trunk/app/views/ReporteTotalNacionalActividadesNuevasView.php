...
class ReporteTotalNacionalActividadesNuevasView {
    ...
    public function showAll($Reporte) {
        ...
        $Actividad_Formato = new ActividadFormatoModel;
	    ...
	    <table width="100%">
	      ...
	      <tr>
	        ...
	        <td><?php echo FormHelper::selectBox('actividad_formato', '', $Actividad_Formato->Find('1=1 order by nombre'), 'codigo', 'nombre', true, array('class' => 'validate-selection selectArea', 'onChange' => 'updateReport()')); ?></td>
        ...
        ?>
        <script>
        parent.Windows.getFocusedWindow().setTitle('Reportes :: Total Nacional Actividades Nuevas');
...
/*
        ...
        function updateReport() {
            new Ajax.Updater('div-list', '<?php echo WEB_PATH; ?>/ReporteTotalNacionalActividadesNuevas/updateList', {
                parameters: { centro_familiar: $('centro_familiar').value, proyecto: $('proyecto').value, programa: $('programa').value, actividad_formato_codigo: $('actividad_formato').value, fecha_inicio: $('fecha_inicio').value, fecha_termino: $('fecha_termino').value},
                onLoading: function(){ TransparentMenu.show('ajax_info_message', {hideMode:'none', insideElement:{id:'div-list'}, showMode:null}); },
    ...
    public function getList($Reporte) {
        ...
        foreach ($Reporte as $Reporte) {
            ...
            ."\n  <td><div align='left'>{$Reporte->actividad_formato_nombre}</div></td>"
            ."\n  <td><div align='center'>{$Reporte->fecha_inicio}</div></td>"
            ."\n  <td><div align='center'>{$Reporte->fecha_termino}</div></td>"
            ."\n  <td><div align='center'>{$Reporte->cantidad_actividades}</div></td>"
            ."\n </tr>";
            
            $cantidad += $Reporte->cantidad_actividades;
        }
        $html .= "\n <tr>"
